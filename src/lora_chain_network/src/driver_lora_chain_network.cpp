...
LoraReturnedStatus DriverStim300::setID(uint8_t * id) const noexcept{
  
}
LoraReturnedStatus DriverStim300::softReset() const noexcept{

}
LoraReturnedStatus DriverStim300::join() const noexcept{

}
LoraReturnedStatus DriverStim300::status(LoraNodeStatus * status) const noexcept{

}
LoraReturnedStatus DriverStim300::send(uint8_t * target, QosLevel qos, uint8_t * payload, int len) const noexcept{

}
LoraReturnedStatus DriverStim300::getChanInfo(ChanInfoList_t * list) const noexcept{

}
LoraReturnedStatus DriverStim300::registerRXcallback(ReceiveCallback cb) const noexcept{
...
LoraReturnedStatus DriverStim300::readOneLine(){
  ...
  while (serial_driver_.readByte(byte)) {
    ...
    if(rxbuffer_.size()>=2 &&rxbuffer_[rxbuffer_.size()-2] =='/r' &&rxbuffer_[rxbuffer_.size()-1] =='/n'  ){
      break;
    }
  }
...
LoraReturnedStatus DriverStim300::sendCommand(char * data,uint16_t len){
    for(uint16_t i =0;i<len; i++){
      serial_driver_.writeByte((uint8_t)data[i]);
    }
}
