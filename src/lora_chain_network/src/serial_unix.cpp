...
SerialUnix::SerialUnix(const std::string &serial_port_name,
                       lora_chain_network_const::BaudRate baudrate) {
...
void SerialUnix::open(const std::string &serial_port_name,
  ...
  bool failure;
  switch (baudrate) {
  case stim_const::BaudRate::BAUD_115200:
    failure = (cfsetispeed(&config_, 115200) < 0 ||
               cfsetospeed(&config_, 115200) < 0);
    break;
  default:
    failure = true;
  }
...
void SerialUnix::close() { ::close(file_handle_); }
